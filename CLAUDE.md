...
**Known residual gap (NOT fixed by this change):** `db/backfill_pipeline.js`'s `booking` CTE still only recognizes `quotes.status='booked'`, not `jobs` — a customer whose only path to `'booked'` is a job (never a real Sales quote) can still be silently re-demoted by this script on a **future deploy**. Guards (1)+(2) prevent the *symptom* (dormant demotion, missing-on-edit) but not this specific *cause*; if it recurs, extend the `booking` CTE's `is_booked` to `BOOL_OR(q.status='booked') OR EXISTS(SELECT 1 FROM jobs ...)`.
**Rules:**
...
3. Any script/job that runs unconditionally on every deploy and can silently change a stage/status column should also log to that column's audit table (`pipeline_history` here) — skipping it (as `backfill_pipeline.js` Step 0 does) makes exactly this kind of bug nearly undiagnosable without a live DB query.
