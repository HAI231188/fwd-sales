...
| BBBG | `GET /api/jobs/:id/bbbg-data`, `POST /:id/bbbg-pdf` | `services/bbbg-pdf.js`, `BBBGModal.jsx` — generate-on-demand delivery-handover PDF (no persistence). Role: `truong_phong_log` + `dieu_do`. **Classic KSVINA layout (redesigned 2026-07-24):** all 3 entrypoints (`buildBbbgPdf` manual / `generateSingleBookingBBBG` email / `generateMultiBookingBBBG`) share ONE `renderClassicBbbg` renderer. **Font = Tinos** (Times New Roman-metric serif, full Vietnamese diacritics) bundled at `assets/fonts/Tinos-{Regular,Bold,Italic}.ttf`, aliases R/RB/RI; **fallback chain Tinos→Roboto→Helvetica** (Roboto TTFs kept so VN still renders if Tinos missing — never Helvetica-first). Enlarged **230pt logo** with a **dynamically-measured header bottom** (`doc.openImage` real drawn height, no hardcoded vertical reserve → no dead space before the rule); **page-centered title** (full page width, not the letterhead block); container table with full cell borders + auto-expanding row heights for long descriptions; **"Thông tin xuất hóa đơn" block moved BELOW Ghi chú** (before signatures) with a page-break guard; **Ghi chú prints only the real note** (the hardcoded-SLB-text double-print/contradiction bug removed); **compressed row pitches** (delivery 19→16, invoice 20→18 + tightened gaps) to fit exactly ONE page. Reads `truck_bookings` (earliest by planned_datetime) + invoice info (L15). |
