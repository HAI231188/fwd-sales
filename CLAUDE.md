...
### Note — DD "Quản lý đặt xe" TK column + ghi chú kế hoạch (2026-08-08)
...
- **"Cần chốt kế hoạch" prompt** — shows when `tk_status IN ('thong_quan','giai_phong','bao_quan')` (all three post-clearance states count the same) **AND** `truck_booking_status === 'chua_dat_kh'` (the existing `get_truck_booking_status()` state, L20/L25). **Deliberately reuses that existing derived state instead of counting containers/bookings itself** — the L38/L39 lesson: a proxy standing in for the real business-intent signal breaks the moment a row exists for a reason the proxy didn't anticipate. `chua_dat_kh` means *zero* containers are linked to any live booking; the instant even one container gets a plan, the state becomes `dat_kh_1_phan` and the prompt clears on its own — **full multi-container coverage is deliberately NOT required**, since DD is already actively working the job once any one container has a plan.
