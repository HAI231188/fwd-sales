...
| BBBG | `GET /api/jobs/:id/bbbg-data`, `POST /:id/bbbg-pdf` | `services/bbbg-pdf.js`, `BBBGModal.jsx` — generate-on-demand delivery-handover PDF (no persistence). Role: `truong_phong_log` + `dieu_do`. **Classic KSVINA layout, finalized 2026-07-30:** all 3 entrypoints (`buildBbbgPdf` manual / `generateSingleBookingBBBG` email / `generateMultiBookingBBBG`) share ONE `renderClassicBbbg` renderer. **Header = a single pre-made banner image** `assets/slb_header.png` (logo + full SLB company info already laid out) — replaces the former hand-drawn logo + letterhead text block. The PNG has transparent padding baked in, so it's drawn via a measured-content-bbox **clip + proportional scale** (no distortion) spanning the **full content width**, top-anchored at `doc.page.margins.top` (no gap above, no horizontal rule below). Title `BIÊN BẢN GIAO HÀNG` / `(Proof of Delivery)` stays **hand-drawn** in Tinos, page-centered (full page width), sitting close under the header image. **Font = Tinos** (Times New Roman-metric serif, full Vietnamese diacritics) bundled at `assets/fonts/Tinos-{Regular,Bold,Italic}.ttf`, aliases R/RB/RI; **fallback chain Tinos→Roboto→Helvetica** (Roboto TTFs kept so VN still renders if Tinos missing — never Helvetica-first). Container table has full cell borders + auto-expanding row heights for long descriptions; **"Thông tin xuất hóa đơn" block sits BELOW Ghi chú** (before signatures) with a page-break guard; **Ghi chú prints only the real note** (the hardcoded-SLB-text double-print/contradiction bug removed); **compressed row pitches** (delivery 19→16, invoice 20→18 + tightened gaps) to fit exactly ONE page. Reads `truck_bookings` (earliest by planned_datetime) + invoice info (L15). |
...
- `GET /api/jobs/` → `tk_notes`, `truck_delivery_location`, `cus_name`, `ops_name`, `tk_status`, `tq_datetime`, `truck_completed_at`, `import_export`, `mail_groups_total`, `mail_groups_unsent`
...
- **Shared helpers:** `ddPillInfo`/`ddPillStyle` in `frontend/src/utils/truckBookingStatus.js` (DD + TP). **TP's 3-dept status `deptStatusLines`/`DeptStatusCell` (formerly inline `tpStatusLines`/`TpStatusCell`) now lives in `frontend/src/utils/jobDeptStatus.jsx` — ONE source of truth shared by BOTH `LogDashboardTP` (Trạng thái column + Đang làm/Hoàn thành partition) AND `SalesDashboard`'s "Quản lý công việc" pending table (Trạng thái column after "Ngày KH giao", desktop + mobile card), so the two screens can't drift (L30). It imports the shared `ddPillInfo`; `TK_TERMINAL` moved there too (TP imports it back for `opsTaskShortStatus`).** Each dashboard keeps its own remaining dept-status helpers (`cusStatusInfo`/`cusWaitingStatus`/`cusIsDone`; `opsStatusKhu1/2`+done predicates; DD `waitingStatus`).
...
### L36 — Pipeline `stage` can silently diverge across THREE independent writers; "has a job" must be a permanent state, not inferred from a timestamp
A customer ("THT Tiêu Dùng") had 5 jobs (one still `pending`) yet `customer_pipeline.stage='dormant'` — invisible to customer-search (`stage='booked'` filter) and the "Data khách hàng" page. Root cause: `customer_pipeline.last_activity_date` only advances on **Sales-side** report saves (`routes/reports.js`), never on **LOG-side** job creation/edits — so a customer with live, ongoing jobs but no *recent Sales report* silently goes stale and `applyAutoTransitions` (L-note above) dormant-flips it every 7 days, regardless of active work. A third writer, `db/backfill_pipeline.js` (runs on **every deploy**, `start.js`), independently recomputes `stage` from a `booking` CTE that checks **only `quotes.status='booked'`** (Sales-side) — zero awareness of `jobs` — and silently overwrote this customer's job-derived `'booked'` back down to `'following'` **without logging `pipeline_history`** (only rows with zero existing history get a history seed), which is why the audit trail had an unexplained gap.
**Fix (2026-07-31):** (1) `applyAutoTransitions`'s dormant-candidate query (`routes/pipeline.js`) gained `AND NOT EXISTS (SELECT 1 FROM jobs j WHERE j.customer_id=cp.customer_id OR LOWER(j.customer_name)=LOWER(cp.company_name))` — **ANY** job (completed, pending, even cancelled/soft-deleted) permanently exempts a pipeline row from dormant demotion; the 7-day-stale rule now applies only to genuine zero-job leads. (2) `PUT /api/jobs/:id` now mirrors the POST "L14" upsert (`jobs.js:~1638-1673`) just before `COMMIT` — any job edit re-asserts `stage='booked'` + `last_activity_date=CURRENT_DATE` using the row's FINAL `sales_id`/`customer_name` (not `req.body`, since unrelated-field edits must still refresh it). (3) One-time backfill promoted the 2 pipeline rows found wrongly stuck `dormant`-with-jobs (SKYBOW id 6997, THT id 7259) to `'booked'`.
**Residual gap — FIXED 2026-09-14:** `db/backfill_pipeline.js` Step 0 used to recompute `stage` from quotes only and silently knocked job-bearing customers from `booked` to `following` on every deploy (5 found live: hà yến, Intech, HOA LILY, hve, hồng quảng — each with a `∅→booked` history row and no record of the demotion). Step 0 now sets every live pipeline row whose customer has ANY job to `'booked'` — the same `hasAnyJobSql` predicate as guard (1), now shared from `services/pipeline-ownership.js` — so it never demotes one and restores the ones already demoted; rows without jobs get exactly the old recompute; every Step 0 change is logged to `pipeline_history`. Its CTEs also count a `customers` row toward the owner of its linked live pipeline (`COALESCE(lp.sales_id, c.user_id)`), so a customer reassigned by L14 is not re-created for the previous owner. NOT fixed by it: job names with no pipeline row at all (e.g. jobs with `sales_id` NULL) — the backfill only recomputes existing rows.

**Stage is derived PER QUOTE — booked is permanent, and a newer interaction never undoes it (2026-09-15).** The promotion rule Step 0 enforces is now stated in full and explicitly: **a customer is `'booked'` if ANY quote in their thread is booked OR they have ANY job, and once booked they are never demoted out of it.** The newest-`interaction_type` recompute (`correct_stages`) decides **only** among `new`/`following`/`dormant`, and **only** for customers that have never had a booked quote and never had a job. The booked-quote half is now its own predicate, `hasBookedQuoteSql` in `services/pipeline-ownership.js` (companion to `hasAnyJobSql`; Step 0 is its only call site), rather than being left to `correct_stages`' `booking` CTE — that CTE reaches a row only through `latest_customer`'s `JOIN reports … deleted_at IS NULL` plus an `(owner, LOWER(TRIM(name)))` grouping, so it returns **no booked verdict at all** for a row whose reports are all soft-deleted (2 live: `PAX VIỆT NAM` 1646, `TRUNG ĐỨC` 1648 — today saved only by `COALESCE(cs.stage, cp.stage)` and by having jobs). The predicate matches a quote to the row three ways because no single one survives every path: `customers.pipeline_id` (the link that survives an L14 transfer, since `customers.user_id` deliberately is not rewritten), `cp.customer_id`, and author+name — the name branch **scoped by owner** (L14), unlike `hasAnyJobSql`, whose "has ever shipped" is a company-level fact.
**Step 1 (INSERT) deliberately gets no matching guard:** `correct_stages` already evaluates `WHEN b.is_booked THEN 'booked'` first, so a brand-new row with a booked quote is inserted `booked`. The guard exists to stop a **demotion**, and only Step 0 can demote.
**Measured on live data before shipping (233 live rows):** **0** rows had a booked quote while sitting below `booked`, so the widened rule promoted **nobody new** — `old_target <> new_target` for **0 of 233** rows, and the 132 rows with neither fact were **bit-identical** under both rules (the explicit "must not change" check). The rule is **preventive**, exactly like the `hasAnyJobSql` half: it makes "booked is permanent" structural instead of a side effect of one `CASE` branch ordering inside a fragile three-way CTE join. The one behavioural change on this deploy is the job half restoring 4 rows (`hve` 63, `Intech` 6013, `HOA LILY` 6014, `hồng quảng` 8818). **`HÀ YẾN` (1603) was in the original list of 5 but is already `booked` again** — a `PUT /api/jobs/:id` re-assert (which writes `stage='booked'` with **no** `pipeline_history` row) silently restored it between the two sessions; its history still shows the unlogged `booked→following` demotion, then `following→dormant`, then a manual `dormant→following`, and no record at all of the promotion that put it back. That silent, unlogged re-assert is the same audit hole rule 3 below is about, still open on the PUT path.
**`applyAutoTransitions` was deliberately NOT widened.** Its only demotion targets `stage IN ('new','following')`, so it can never demote a `booked` row; and a booked-quote row parked at `following` between deploys ends at `booked` anyway once Step 0 runs. Widening it would change rows the "neither" constraint says must not move.
**Rules:**
1. When customer/pipeline **visibility** state (stage, status, dormant/active) can be independently written by more than one subsystem (Sales reports, job creation, job edits, a deploy-time backfill script), audit **all** writers before trusting any single one's logic — a silent, unlogged overwrite from an unexpected writer (here, a deploy script) is easy to miss.
2. A derived "customer is active" signal should never depend solely on a timestamp advanced by only ONE department's actions (here, Sales reports) when OTHER departments (LOG jobs) also constitute genuine activity — either broaden what advances the timestamp, or (safer, done here) make the has-a-job condition an unconditional structural exemption instead.
3. Any script/job that runs unconditionally on every deploy and can silently change a stage/status column should also log to that column's audit table (`pipeline_history` here) — skipping it (as `backfill_pipeline.js` Step 0 did until 2026-09-14) makes exactly this kind of bug nearly undiagnosable without a live DB query.

### L37 — `booking_code` must embed the job's unique `id`, not just its (non-unique) `job_code`
DD's "Đặt kế hoạch xe" save failed for `LG26070043` with `duplicate key value violates unique constraint "idx_truck_bookings_booking_code_uniq"`. Root cause: `nextBookingCode` (`routes/truck-bookings.js`) generated `booking_code = "KH-{job_code}-{NN}"` — derived from `job_code`, a human-readable label with **no DB uniqueness constraint** (`jobs.job_code` has no UNIQUE index; it's client-supplied at `POST /api/jobs`, unvalidated for dedup). Two job ROWS can share a `job_code` (e.g. one soft-deleted, one active, both real cases found live) — each independently computes the identical first booking_code (`"KH-{job_code}-01"`), colliding on the global unique index (not scoped by `job_id`).
**Fix (2026-08-02):** `booking_code` format is now `"KH-{job_code}-{job_id}-{NN}"` — embedding the job's own unique numeric `id` makes every generated code collision-proof regardless of `job_code` duplicates, past or future. NN stays sequential **per job**, scoped by the `job_id` **column** (`WHERE job_id = $1`) — not a `LIKE` on the booking_code string shape. (A first draft added `AND booking_code LIKE '...-{job_id}-%'` for "extra" scoping; caught in verification — it silently missed pre-existing OLD-format rows on the same `job_id`, restarting NN at 01 instead of continuing the sequence. The `job_id` column filter alone is exact and sufficient; the LIKE was redundant and broke continuity once the string format changed.) The `'\d+$'` NN-extraction regex is unaffected (job_id sits before the final `-`, so it still isolates only NN). Fixes `LG26070043`, `LG26060126`, and the `LG26070010` pair — all self-heal on their next save, no backfill needed (`nextBookingCode` only ever runs at save-time; nothing to migrate for existing rows).
**Flagged, not fixed:** `jobs.job_code` itself having no UNIQUE constraint (and being client-supplied) is a *separate* root cause — this fix stops the booking collision but doesn't stop two jobs from sharing a code in the first place. Noted for a future investigation.
**Rules:**
1. Never derive a uniqueness-critical generated value (a code, a slug, a filename) from a column that isn't itself guaranteed unique — if the source can duplicate, embed the row's real primary key instead.
2. When adding a scoping condition to a generator query, prefer an exact column filter (`WHERE job_id = $1`) over a `LIKE`/string-shape filter — the latter silently breaks once the generated format changes, exactly as happened here.
3. Before shipping a "collision-proof" fix, actually test it against real historical rows with the OLD format still in the table — a fresh-row-only test would have missed the NN-continuity regression this one caught.

### L38 — job-container edits must never cascade-delete a live booking's link; DD's "đã đặt KH" must branch on `cargo_type`, not container count
Job `LG26060163` had a fully-complete truck booking (carrier, cước, số xe, mail sent, both ticks) yet showed "Chưa đặt KH" and couldn't complete. Two independent bugs, found together (6 jobs affected initially, 8 on a full recount):
**Bug A (data loss via cascade):** `PUT /api/jobs/:id`'s container-edit block did `DELETE FROM job_containers WHERE job_id=$1` unconditionally, then re-inserted. `truck_booking_containers.container_id REFERENCES job_containers(id) ON DELETE CASCADE` — so ANY job edit that resubmitted containers (even re-saving the same values) silently deleted the link row for an already-complete booking, without touching the booking itself. `job_history` proved this: 4 of 5 affected jobs had a `containers` edit **after** their booking's `created_at`, four of them clustered within 90 seconds of each other on the same day.
**Bug B (LCL logic mismatch):** `get_truck_booking_status()` branched on `a_total_cont = 0` (container count) to decide whether a job needed link-coverage. An LCL job (`cargo_type='lcl'`) legitimately books container-less (the bulk "Đặt kế hoạch xe" endpoint's cross-check *forbids* a container_id on an LCL row) — but if that job also carries a reference `job_containers` row (entered separately, e.g. for tracking), the count-based branch wrongly forced it through the FCL coverage check, which a deliberately-unlinked LCL booking can never satisfy.
**Fix (2026-08-03):** (1) The PUT container-edit block now **reconciles** instead of wiping — matches incoming rows to existing ones by content (`cont_number`/`cont_type`/`seal_number`), keeps matched rows untouched (id + any link survive), inserts only genuinely new rows, and deletes an unmatched row **only if no live booking links to it** (a linked row is preserved + logged via `recordHistory('container_kept_linked', ...)`, mirroring the L35 "OPS-completed KEEP" pattern — never destroy committed work). (2) `get_truck_booking_status()` branches on `job_cargo_type = 'lcl'` instead of container count; the FCL/non-LCL branch is unchanged, still requires full link coverage.
**Rules:**
1. Before making a DELETE-then-reinsert pattern on any table, check what other tables reference it via `ON DELETE CASCADE` — a "just resave the same data" edit can silently destroy dependent rows nobody expects it to touch.
2. When two derived states can each independently justify "coverage required" (here: container *count* vs `cargo_type`), pick the one that reflects true business intent (`cargo_type`) — a count-based proxy breaks the moment a row exists for a reason unrelated to the check (a reference container on an LCL job).
3. `job_history` timestamps are the forensic tool for "was this a fresh bug or an edit-triggered regression" — a cluster of edits across unrelated jobs within seconds of each other is a strong signal of a batch operation, not independent user actions.

**One-time data repair (2026-08-03) — re-inserted the missing `truck_booking_containers` link rows for the 5 already-complete bookings the cascade bug had orphaned:** `LG26070028`(149) booking 327↔container 546, `LG26070086`(192) booking 312↔container 547, `LG26070129`(221) booking 289↔container 473, `LG26070119`(208) booking 320↔container 548 + booking 321↔container 549 (2-container job, paired by insertion order — no distinguishing data existed between the two bookings, flagged as a heuristic at the time). Dry-run first (fresh id re-verification, before/after `get_truck_booking_status()` + DD-card counts, jobs-75/117 isolation proof), then approved and committed. Confirmed: DD card `da_dat`/`chua_dat` moved by exactly ±5 (17→22 / 41→36), nothing else shifted; jobs 75/117 stage `hoan_thanh` unchanged before/after.
**Job `LG26060163`(101) needed no repair** — Bug B's `cargo_type` fix alone flipped it to `du_xe_cho_giao`, and it was later completed by DD through normal use.
**Job `LG26070122`(214) resolved itself before the repair ran** — real DD activity same-day soft-deleted the old orphaned booking and created a *new* one (`booking_code = "KH-LG26070122-214-02"`), correctly linked from creation, and completed the job. This incidentally confirmed the L37 collision-proof `booking_code` format (`KH-{job_code}-{job_id}-{NN}`) working correctly on a live, organic save. Excluded from the repair as unnecessary.
**Jobs 75 (`LG26060137`) and 117 (`LG26060178`) excluded — genuinely ambiguous, not guessed:** job 75's two containers have identical content (`cont_number` both `null`, same `cont_type`) and its two bookings differ only by vehicle plate — no signal to pair either. Job 117 has 4 containers vs 4 live bookings with zero container-identifying data in any booking — a 4-way ambiguity, not resolvable by the same insertion-order heuristic used for job 208.

**Follow-up fix (2026-08-04) — id-based container matching, closing the cont_number-edit gap (deferred item 2 below).** The 2026-08-03 content-signature reconcile fixed Bug A but left a narrower gap: editing only `cont_number` on an already-linked container isn't recognized as "same row" by content signature, so the reconcile keeps the stale row (still linked, preserved by the KEEP-guard) AND inserts the edited value as a brand-new, unlinked row — same uncompletable symptom (`b_booked_cont < a_total_cont` in `get_truck_booking_status()`), reached through a routine cont_number correction instead of a full resave, with no self-healing path (there is no real second container to ever book). **Proven live** on job 265 (`LG26070199`, `BEGIN…ROLLBACK`, nothing persisted): the edit alone flipped status `du_xe_cho_giao → dat_kh_1_phan`.
**Forensic scan (2026-08-04) found zero existing damage.** Scanned all 139 non-LCL jobs with ≥1 live booking; 0 are currently coverage-capped (`dat_kh_1_phan`/`chua_dat_kh`) — sanity-checked separately (system-wide status distribution, a raw unlinked-container query, live-booking counts) to rule out a false negative before reporting the clean result. No repair was needed.
**Fix:** `PUT /api/jobs/:id`'s container-edit block (`jobs.js:2286-2372`) now `UPDATE`s a `job_containers` row **in place** when the incoming payload carries an `id` that belongs to this job — the row's identity (and therefore its `truck_booking_containers` link) never moves. Guards:
1. **Ownership check** — the id-lookup map is built only from `job_containers WHERE job_id=$1`, so an id belonging to another job can never match; the `UPDATE` statement also repeats `AND job_id=$5` as an independent second check.
2. **Whole-payload legacy fallback** — id-mode only activates if the payload owns at least one id belonging to this job; a payload with **zero** owned ids falls through entirely to the pre-existing 2026-08-03 content-signature algorithm, unchanged. **A user on a stale cached frontend bundle (predating this fix, which never sends `id`) still hits the narrower cont_number-edit gap until their browser reloads the new bundle** — this fix is not retroactive to already-open tabs.
3. **Partial-id payloads** — inside id-mode, an id-less row (a newly-added container) always falls to `INSERT`, never to signature-matching — so it can never accidentally claim (and thereby save from deletion) an existing id-bearing row.
4. **KEEP-guard unchanged** — an unclaimed row with a live booking is still preserved + `recordHistory('container_kept_linked', ...)`, exactly as the 2026-08-03 fix left it.
Frontend (`JobDetailModal.jsx:451-452`) now includes `id: c.id` when building `draft.containers` from the already-fetched `job.containers` — the id was always returned by `GET /api/jobs/:id`, just previously dropped when building the edit draft.
**Proof — all 4 required cases, real jobs, `BEGIN`+`SAVEPOINT`+`ROLLBACK`, nothing persisted:** (1) cont_number-only edit on job 265 → `UPDATE` in place, link intact, status **stays** `du_xe_cho_giao` — the regression is fixed. (2) resubmit identical content on job 265 → no-op, before/after snapshots identical. (3) add a genuinely new container on job 282 (`LG26080012`) → existing linked row+link untouched, new row inserted unlinked, status correctly drops to `dat_kh_1_phan` (real business state — a genuinely un-booked container — not a bug). (4) submit an empty payload on job 281 (`LG26080011`) → KEEP-guard fires, linked row preserved, status unchanged. **Re-proven post-deploy** (commit `4e0c12f`): the deployed reconcile block was byte-diffed identical to the tested code, then case 1 was re-run live against the running build and its DB — status confirmed staying `du_xe_cho_giao`.
**`weight_tons` gap noticed in review, not fixed here:** the PUT reconcile's `UPDATE`/`INSERT` column lists have never included `weight_tons` (present in both the pre-2026-08-03 code and this id-mode branch — copied forward unchanged), even though `POST /api/jobs` (create) and `GET /:id` both handle it and the frontend lets users edit it. Editing an existing container's weight via PUT silently no-ops; adding a new container with a weight via PUT always saves `NULL`. Confirmed pre-existing, deliberately left alone this deploy — a separate fix.

**Deferred (not lost — pick up later):**
1. **Reconcile regression proof never completed.** Three attempts at invoking the actual `PUT /:id` handler in-process (extracting the registered function directly, intercepting `COMMIT` to prevent persistence) stalled indefinitely at the same point — after acquiring a DB client, before the first `client.query()` — for an unconfirmed reason; a minimal harness sanity-check proved the interception mechanism itself works, so the stall is specific to the handler invocation. Explicitly dropped in favor of real-world DD usage as the substitute proof; direct-SQL `BEGIN…ROLLBACK` reproduction against real jobs (proven to work well — see the cont_number-edit fix above) is the standing alternative if a future regression needs checking.
2. ~~`cont_number`-edit failure mode, undiagnosed.~~ **RESOLVED 2026-08-04.** Confirmed exactly as hypothesized, fixed via id-based matching, live-proven pre-deploy (job 265) and re-proven post-deploy against the running build. See the follow-up fix above.
3. ~~Only `PUT /api/jobs/:id` was audited/fixed for the cascade-delete pattern.~~ **RESOLVED 2026-08-04.** `grep -rn "job_containers" backend/src` (guard e of the 2026-08-04 fix) confirms exactly two writers exist system-wide: `POST /api/jobs` create (`jobs.js:1629`, INSERT-only) and this PUT reconcile block. Nothing else was ever exposed to the cascade-delete pattern; nothing left to audit.
4. **A second, structurally identical instance of the Bug-B proxy class, found but not fixed — the one open item.** The DD "Kế hoạch đã đặt / chưa đặt" cards (`keHoachDaDat`/`keHoachChuaDat`, `routes/jobs.js`) use a container-level `EXISTS` on `truck_booking_containers` with **no `cargo_type` branch** — so a pending LCL job carrying a reference container row can still land in "chưa đặt" on this card even when `get_truck_booking_status()` correctly reports it as planned. Confirmed via live query; not fixed. **Display-only — does not block job completion** (`get_truck_booking_status()`, the actual completion gate, is unaffected and already branches on `cargo_type` correctly).

Items 2 and 4 are both the same lesson already captured in Rule 2 above: **a proxy (container count, link existence) standing in for true business intent (`cargo_type`) breaks the moment a row exists for a reason unrelated to the check.** Two confirmed instances now — assume more exist; audit any other place that infers "is this job's truck side handled" from container/link presence rather than reading `cargo_type` directly.

### L39 — A "validate each item" completeness check is not a "require at least one item" check
`containers.some(c => incomplete)` (the hàng nhập FCL row-completeness guard, `CreateJobModal.jsx:301-310`) validates only rows that exist; it never asserts a row exists. `[].some(...)` returns `false`, so a submission with zero container rows passes silently — exactly how `LG26070027` (job 147, FCL+import+`both`) was created with no `job_containers` row: `job_history` shows no `containers` entry at creation, confirming `containers: []` was submitted and the existing check let it through vacuously.
**Fix (2026-08-06):** a new guard requires **at least one** container row with a non-blank `cont_number` for the exact combination `cargo_type='fcl' AND import_export='import' AND service_type IN ('truck','both')` — mirrored on `CreateJobModal.jsx` submit (`setInvoiceErr`, same render slot as the sibling check — outside the scrollable `.modal-body`, pinned directly above "Tạo Job") and `routes/jobs.js` POST `/` (placed before the transaction, so a rejection touches zero DB rows/sequences). **Three exemptions, all intentional:** hàng xuất (`import_export='export'` — the container doesn't exist until the empty is picked up post-cutoff); LCL (`cargo_type='lcl'` — whole-lot leg, no container link, L20/L38 precedent); no-truck (`service_type IN ('tk','ops_hp')` — nothing for DD to plan). **Create-only** — `PUT /api/jobs/:id` is a structurally separate handler and never runs this guard, so the 14 live jobs with zero `job_containers` rows stay editable.
**Rule:** `.some()`/`.every()` over a collection is a **completeness** check on existing items, not an **existence** check on the collection itself — an empty array trivially satisfies both `[].some(bad) === false` and `[].every(good) === true`. When a business rule actually requires "at least one X", assert that explicitly (e.g. `arr.length > 0 && arr.some(...)`) — never assume a completeness guard implies it.
**Known gap, not fixed:** the legacy fallback container editor still exists (`JobDetailModal.jsx` — a three-field quick-editor writing the deprecated scalar `jobs.cont_number`/`cont_type`/`seal_number` columns directly, independent of `job_containers`), and this guard only covers **creation** for the FCL+import+truck/both combination. Hàng xuất and LCL jobs (exempt by design) — and any job edited post-creation through that legacy scalar editor rather than the real `job_containers` array — can still fall into the same class of trap: data that displays somewhere (the scalar column, or a stale LÔ HÀNG panel read) without a real, bookable `job_containers` row underneath it. Not addressed here; flagged for a future pass.

### L40 — Hạn lệnh guard on truck-plan saves: compare VN CALENDAR DATES, never raw timestamps
(Shipped 2026-09-07, commit `43b4c68`.) A truck-plan save whose delivery date lands on a **later Vietnam calendar day** than the job's `jobs.han_lenh` is **refused**. This is a **data-correctness** mechanism, not a demurrage guard: free-time extensions are granted in real life but never entered into the app, so a stale `han_lenh` silently poisons every downstream signal that reads it (the DD **T1 "Quá hạn đặt KH xe"** tier, BBBG, the planning mail). Blocking the save forces the responsible department to correct the value. **Every late save is blocked — no grace threshold; 1 day over is still blocked** (owner decision; ~24% friction on in-scope saves accepted).

**Single predicate, all THREE writers.** `backend/src/services/han-lenh-guard.js` (L30 — one home, never re-copied) exports `checkPlansAgainstHanLenh(client, checks)` plus `isPlanLate` / `isSameVnDate` / `hanLenhUsable` / `jobInGuardScope` / `vnDateStr`. It is called by **all three** endpoints that write `truck_bookings.planned_datetime` — `POST /`, `PATCH /:id`, `POST /batch` (`routes/truck-bookings.js`). **Guarding fewer than three leaves the block bypassable** — a fourth `planned_datetime` writer must be wired in too. Each call sits **before** any write (and before `nextBookingCode`, so a rejection burns no booking-code sequence number), `ROLLBACK`s, and returns `400 {error, code:'HAN_LENH_EXCEEDED'}` naming every offending container in ONE message (whole-save rejection — no partial writes, the L38 failure mode).

**Scope — FCL + hàng nhập only. Three exemptions, all deliberate:**
1. **LCL** (`cargo_type='lcl'`) — a container-less whole-lot leg with no per-container free time (L20/L38).
2. **Hàng xuất** (`import_export='export'`) — for export the same column is a **CUTOFF**, i.e. the opposite concern (L19).
3. **Unusable `han_lenh`** — NULL, unparseable, or VN year outside `2020..2030` ⇒ cannot evaluate ⇒ **ALLOW**. There is a live row with year `0226`; the guard **never blocks on missing or corrupt data**.

**⚠ WHY THE COMPARISON IS ON VN CALENDAR DATES — do NOT "simplify" this back to a raw timestamp compare.** An import `han_lenh` is submitted as a bare `'YYYY-MM-DD'` (an `<input type="date">` value; `vnLocalToIso` deliberately passes date-only strings through untouched), so Postgres parses it at the **session** timezone — `Etc/UTC` on Railway — landing it at **00:00 UTC == 07:00 Asia/Ho_Chi_Minh**. **322 of 324 live import rows sit at exactly 07:00 VN; that 07:00 is a STORAGE ARTIFACT, not a business time.** A raw `planned_datetime > han_lenh` therefore flags **every same-day delivery after 07:00 VN** — and 08:00/13:00/17:00/19:00/21:00 are the common planned times, so that is essentially all of them. Measured against live data the raw comparison over-flags by **42 bookings / 29 jobs (~35% false positives)**. Hạn lệnh is a **DAY** deadline for import: a delivery at **17:00 on the hạn lệnh day is ON TIME**. Comparing the zero-padded VN date key (`"YYYY-MM-DD"`, so plain string compare is chronological) removes the false positives entirely. **Anyone who rewrites this as `planned_datetime > han_lenh` breaks it SILENTLY** — no error, no test failure, just ~a third of legitimate same-day afternoon plans suddenly refused. Proven live: a same-day 17:00 save is allowed by the guard while a raw compare on the identical values returns `true` (would have blocked it).

**The new/changed-date rule — only a NEW or genuinely CHANGED date is validated.** An untouched stored date must never be re-validated, or an existing already-late plan could never be edited again. `POST /` and `POST /batch` always validate (a create's date is always new). `PATCH /:id` validates only when `req.body.planned_datetime !== undefined` **AND** `!isSameVnDate(submitted, stored)`. **"Unchanged" is compared on VN calendar date, NOT string equality:** the client round-trips the stored instant through `toDatetimeLocal` → `vnLocalToIso`, so a submitted `"2026-09-06T08:00:00+07:00"` is textually nothing like the stored `"2026-09-06T01:00:00.000Z"` while meaning the same instant — string equality would treat every resend as a change and block the edit. Comparing on the DATE also makes a time-only tweak (08:00 → 17:00, same day) count as unchanged, which is correct: the verdict depends on the date, so it cannot have moved. **This is exactly what lets a DD keep saving carrier / số xe / cost on an already-late plan.** `TruckPlanningModal` never sends `planned_datetime` at all (the `undefined` check covers it), but `BookingModal` DOES resend the unchanged date — the `isSameVnDate` check is what actually protects that flow.

**`sales` removed from `PLAN_ROLES`** (`constants/roles.js`). A blocked sales user cannot unblock themselves: clearing the block means editing `jobs.han_lenh`, which `canEditJob` (`services/job-access.js`) grants to TP/lead, the assigned CUS, the assigned DD and the owner-sales only — and a sales user is not necessarily the owner of the job they were planning. **Zero-usage evidence:** of **490 `truck_bookings` all-time**, **ZERO** were created by a `role='sales'` user (`dieu_do` 394, `cus1` 75, `cus3` 16, `truong_phong_log` 4, `cus2` 1); only 5 distinct people have ever created one, all `*.log` accounts, while 11 sales users (10 active) exist and none has ever created a booking. No sales-reachable page mounts `PlanDeliveryModal` (it opens only from `LogDashboardCus` / `LogDashboardDieuDo` / `LogDashboardTP`). `PLAN_ROLES` governs exactly **4 sites** — `POST /api/truck-bookings/batch` (via `canPlan`) plus the three read endpoints `GET /api/jobs/:id/{available-containers,past-delivery-locations,past-receivers}` — all the same "Đặt kế hoạch xe" surface, so the cut is confined there. `cus`/`cus1`/`cus2`/`cus3`/`lead`/`dieu_do`/`truong_phong_log` all stay.

**KNOWN GAP — plan rights are role-scoped, `han_lenh` edit rights are assignment-scoped (deliberately NOT fixed).** `POST /batch` gates on `canPlan(req)` = **PLAN_ROLES membership only, with no assignment check** — any CUS can plan *any* job. But `canEditJob` is strictly assignment-scoped for those same roles (`assignment.cus_id === user.id` / `assignment.dieu_do_id === user.id`; only TP/lead are unconditional). **So an unassigned CUS or an unassigned `dieu_do` can be blocked by this guard with no way to fix the `han_lenh` themselves** — the identical dead-end that got `sales` removed. Measured: of 486 bookings created by cus*/dieu_do users, **only 1** was on a job the creator wasn't assigned to (one `dieu_do`; CUS is 0 for 92). Owner decision (2026-09-07): **leave it alone** — 1 occurrence in 486 is too rare to justify touching permissions, and widening `canEditJob` or narrowing `canPlan` risks blocking real work. Recorded, not fixed. If it starts recurring, the cheaper fix is widening the `han_lenh` edit right, not narrowing who may plan.

**KNOWN GAP — the plan-save is still NOT atomic.** `PlanDeliveryModal`'s save fans out to **1 batch POST + N PATCH + M DELETE** with **no cross-request transaction**, so validating mid-flight could leave a PARTIAL save — the L38 failure mode (a DD who believes 4 containers saved when only 3 did is worse off than one who saved none). All-or-nothing therefore rests on a **frontend pre-flight** that validates **every enabled row BEFORE the first request is issued** (`hanLenhViolations` in `PlanDeliveryModal.jsx`, mirroring the backend scope exactly, using `vnDateKey` from `utils/dateFmt.js`; the error renders outside the scrollable `.modal-body`, pinned above the action button, per the L39 `CreateJobModal` precedent). **The backend remains the real gate** — the pre-flight only lets the DD see the problem in the modal instead of after a round-trip, and it cannot be relied on for correctness (a direct API caller bypasses it). **The single-transaction plan-save rewrite remains DEFERRED** — it touches the delete path that caused the L38 container-link damage, so it was explicitly kept out of this change. Until it lands, any new client-side validation on this modal must also run entirely before the first request.

**Rules:**
1. A deadline column stored date-only but typed `TIMESTAMPTZ` must be compared on the **project-timezone calendar date**, never as a raw instant — the stored time-of-day is a session-timezone artifact, and a raw compare silently mis-flags an entire class of legitimate rows.
2. When a guard can refuse a save, verify the blocked user has the rights to clear the block themselves — if not, either grant them, remove them from the writing role (as done for `sales`), or record the dead-end explicitly.
3. Validate only values that are **new or actually changed**, and decide "changed" on the same semantic the verdict uses (here the VN date) rather than string equality — otherwise a round-tripped resend of an unchanged value re-triggers the block and freezes rows already in a violating state.
4. A guard on a multi-request save is all-or-nothing only if the client checks **every** row before issuing the **first** request; a per-request check yields partial saves (L38).

**Verified before ship** — `node --check` on all three backend files, `npm run build`, and 23 assertions in `BEGIN…ROLLBACK` against live data (including a positive control proving the "nothing written" assertion was not vacuous). **Verified after deploy against the LIVE route** (17 assertions; all writes restored, nothing created): `POST /batch` with a 10-days-late date → `400 HAN_LENH_EXCEEDED`, container named in Vietnamese, zero rows created and the container left unlinked; a same-day 17:00 `PATCH` → `200` and persisted; an already-late booking's `vehicle_number` edit → `200` with `completed_at` and `planned_datetime` undisturbed.

### Note — DD "Quản lý đặt xe" TK column + ghi chú kế hoạch (2026-08-08, prompt widened 2026-08-10)
One merged column in `BookingManagementSection`/`BookingRow` (`LogDashboardDieuDo.jsx`), header **"TK / Ghi chú KH"**, placed right after "Trạng thái" — pairs TK clearance state with truck-plan state so DD reads both together (that pairing is the whole point of the prompt below).
- **Ngày giờ TK + trạng thái TK** — reads `j.tk_status`/`j.tk_datetime`, already present on every row via the shared `GET /api/jobs` `j.*` SELECT; **no query change needed** (L31 point 5 — `j.*` auto-returns new/existing columns). Local `TK_STATUS_LABEL`/`TK_STATUS_COLOR` map (L30 exception — per-file display-enum copy, mirrors `JobDetailModal`'s).
- **"Cần chốt kế hoạch" prompt** — shows when `tk_status IN ('dang_lam','thong_quan','giai_phong','bao_quan')` (widened 2026-08-10 to include `dang_lam` — only `chua_truyen` stays exempt) **AND** `truck_booking_status === 'chua_dat_kh'` (the existing `get_truck_booking_status()` state, L20/L25). **Reasoning for the widen:** a declaration already `dang_lam` (in progress) is about to clear, so DD should start lining up the truck plan while it's in progress rather than waiting for clearance. **Deliberately reuses that existing derived state instead of counting containers/bookings itself** — the L38/L39 lesson: a proxy standing in for the real business-intent signal breaks the moment a row exists for a reason the proxy didn't anticipate. `chua_dat_kh` means *zero* containers are linked to any live booking; the instant even one container gets a plan, the state becomes `dat_kh_1_phan` and the prompt clears on its own — **full multi-container coverage is deliberately NOT required**, since DD is already actively working the job once any one container has a plan. Live count went 11→19 (of ~46 truck-side jobs in scope) on widening — a real but not table-dominating jump, checked and accepted at ship time.
- **`TK_PLAN_PROMPT_STATUSES` vs `TK_TERMINAL_STATUSES` — kept deliberately separate, do NOT merge.** They look interchangeable (both are lists of `job_tk.tk_status` values) but mean different things: `TK_TERMINAL_STATUSES = ['thong_quan','giai_phong','bao_quan']` feeds `waitingStatus()`'s "CUS thông quan" Chờ-column blocker, where `dang_lam` **must** stay NOT-terminal (a declaration still in progress genuinely means CUS still owes work — the blocker should keep showing). `TK_PLAN_PROMPT_STATUSES = ['dang_lam','thong_quan','giai_phong','bao_quan']` feeds only `ddPlanPrompt()` above, where `dang_lam` **must** count as prompt-worthy. A future "tidy up, these two lists are basically the same" merge would silently break the Chờ column (a job with TK `dang_lam` would stop showing "CUS thông quan" even though CUS hasn't finished). Both constants live at the top of `LogDashboardDieuDo.jsx`, right next to each other — same file, same section, on purpose, so this note and the inline comments stay easy to find together.
- **`jobs.dd_plan_note`** — new `TEXT` column (idempotent `ALTER TABLE jobs ADD COLUMN IF NOT EXISTS`, Golden Rule 3), mirrors the `ops_hp_note` precedent: a plain column directly on `jobs`, not a child table, since the note is 1:1 with the job and never a list. DD's free-text scratchpad for a customer who hasn't confirmed a schedule; independent of `job_tk.notes` ("Ghi chú TK", CUS-owned — DD has no write access to that field). Inline-edited via the file's existing `InlineInput` component (no modal).
- **`PATCH /api/jobs/:id/dd-plan-note`** (`routes/jobs.js`) — gated by `canEditDdPlanNote(user, assignment)` (`services/job-access.js`): assigned DD (`assignment.dieu_do_id === user.id`) or TP/lead; every other role 403. Mirrors `canEditJobTk`'s shape (assigned-dept + TP/lead) but scoped to `dieu_do` — **deliberately does NOT reuse the broader `canEditJob`**, which also grants the assigned sales/CUS user edit rights, wrong for a DD-only field. Every write logs via the existing generic `recordHistory(client, jobId, userId, 'dd_plan_note', old, new)` — same helper every other job-field PATCH uses, distinct `field_name`.
- **`api/index.js`**: `updateDdPlanNote(jobId, note)`. The mutation invalidates `['jobs']` — partial-key match catches `['jobs','pending']` (TanStack Query default, no `exact:true`), the same pattern the existing revenue-tick endpoint already uses to refresh this same list.
- **"Chưa gửi mail" warning chip (2026-09-08)** — a second ⚠ chip in this same cell, **display-only** (blocks nothing, changes no permission). Shows when the job has ≥1 **carrier-bearing mail group** AND ≥1 of those groups has never been mailed; renders **nothing at all** otherwise. Wording splits on whether anything went out: `Chưa gửi mail` (nothing sent) vs `Chưa gửi mail đủ` (some sent, some not) — "đủ" on a zero-sent job would wrongly imply a partial send. The fraction lives in the `title` tooltip (`Đã gửi mail {sent}/{total} nhóm vận tải`), mirroring `ddPillInfo`'s `Nâng hạ x/y · Cost hệ thống x/y` precedent — on-screen counts were rejected because only **4.3%** of mailable jobs (13/300 live) ever have >1 group, so a fraction would read `0/1` on ~96% of hits.
  - **A mail GROUP = one `(transport_company_id, mail_group_id)` pair** — the same key `statusKey()` uses in `TruckPlanningModal` and the `all_keys` CTE uses in `services/email-status.js`. `mail_group_id IS NULL` = the forming batch = never mailed. That signal is **structurally** safe, not just empirically: a successful `'new'` send is the ONLY writer that sets it non-NULL (`services/email-sender.js`, guarded by `status === 'sent' && mailType === 'new'`, so a **failed** send can never mark a group as mailed), and a carrier change resets it to NULL (`routes/truck-bookings.js`) so a re-carriered booking correctly re-flags as unsent. The only way it could diverge from the stricter `email_history` join is a **soft-deleted** `email_history` row — and nothing in the app ever writes `email_history.deleted_at` (the single `UPDATE email_history` in the codebase patches `last_sent_data`). Verified live at ship time: 271 mail rows, 0 soft-deleted, 0 failed, **0 mismatches** between the loose and strict checks across all 67 candidate jobs.
  - **`mail_groups_unsent` counts DISTINCT carriers, not pairs** — every unmailed booking of one carrier shares `mail_group_id = NULL`, so a carrier has at most one forming batch.
  - **The carrier gate is load-bearing.** A booking with no `transport_company_id` forms no group (`TruckPlanningModal` skips it: `if (!r.booking_id || !r.transport_company_id) continue;`), so it has no card and **no Send button** — marking it would raise a warning the viewer cannot clear. Its real state is already reported one cell over by the Trạng thái pill (`du_kh_chua_chot_vt` / `du_kh_chot_vt_1_phan`); firing here too would double-report one condition and point DD at a button that does not exist. A job with **some** groups carriered and some not therefore shows the chip only for the carriered-and-unmailed ones.
  - **Colour = teal** (`rgba(20,184,166,0.14)` / `#0d9488`), amber (`#d97706`) stays with "Cần chốt kế hoạch". Teal is the one `PILL_COLOR_TOKENS` colour guaranteed never to appear in this table (its only owner, `dd_da_xong`, is excluded by `TRUCK_BOOKING_ACTIVE_STATUSES`), it collides with none of the five `TK_STATUS_COLOR` values rendering in the SAME cell (gray/amber/green/blue/purple), and it leaves **red exclusively meaning "quá hạn"** per the DD 3-tier overdue design. Amber-vs-teal also survives red-green colour blindness, which amber-vs-red would not. Both chips share `warnChipStyle(bg, fg)` (module-level in `LogDashboardDieuDo.jsx`) so only hue distinguishes them.
  - **⚠ KNOWN GAP — ghost / `can_huy` batches are NOT covered.** A sent batch whose bookings have all moved away still owes a **HỦY** mail, and `email-status.js`'s `all_keys` CTE surfaces exactly that by UNION-ing `email_history`. This chip deliberately ignores it: one chip must not mean two different buttons. Measured at ship time: **1 in-table job** (`LG26090010`) was in that state and correctly showed nothing. If cancel-hygiene needs surfacing, add a SEPARATE chip — do not widen this one.
  - **Backend:** two aggregates on `GET /api/jobs`, unconditional (next to `bookings_total_alive` / `bookings_with_invoice_lifting`, same `COALESCE((SELECT COUNT…), 0)` shape) — **not** gated to `role='dieu_do'` like `ddFlags`, so TP can reuse them for an L10 broadcast without a backend change. No schema change: `truck_bookings.mail_group_id` already exists with `idx_truck_bookings_mail_group` + `idx_truck_bookings_job`.
  - **One render site.** `BookingManagementSection` is a hand-rolled `<table>` with `overflowX: auto`, not a `FilteredTable` — it has no `renderMobileCard` twin, so L26 desktop/mobile parity adds no second site here. (The `renderMobileCard` further down the file belongs to the main DD grid.)
  - **⚠ THIS IS A PREVENTIVE MARKER — do NOT judge it by how often it is on screen.** At ship time it rendered on **1 of 61** in-table rows, and it is designed to look like it does nothing: 38 rows had no carrier-bearing group and 22 were fully mailed. That is the marker working, not failing. The stock is low because the value is in the FLOW: of 340 non-deleted truck/both jobs, 56 had an unsent carrier group and **53 of those were already `completed`** — i.e. they crossed DD's table and reached completion with a carrier assigned and no planning mail ever sent, at which point the chip can no longer help. By job-creation month, among jobs that ever had a mailable group: 2026-06 **15**/59, 2026-07 **21**/127, 2026-08 **20**/104 — a steady **~15-21 per month**, and NOT a pre-feature artifact (the email feature went live 2026-06-17, inside that window).
    **The number to watch is whether that ~15-21/month completed-with-unsent-group rate falls** — not the on-screen hit count, which will usually be 0-2. Re-measure with: jobs where `EXISTS` a live booking with `transport_company_id IS NOT NULL AND mail_group_id IS NULL`, grouped by month, restricted to `status='completed'`. If the rate does not move after a few months, the chip is being ignored (wrong placement/colour/wording) or DD is mailing outside the app — investigate that before assuming the signal itself is wrong. Caveat on the baseline: it assumes those 53 jobs had their carrier assigned while still IN the DD table; a carrier attached only after DD completion would not have been catchable, and that was not verified retroactively.
**Rules:** when a highlighted/derived UI signal needs "is X already handled for this job", read the existing per-job derived state (`get_truck_booking_status()`, `tk_status`, etc.) — never recompute a parallel count or existence check (L38/L39, now a third confirmed instance of the same lesson). A DD-owned scratch field's permission model = assigned-dept + TP/lead, mirroring `canEditJobTk`'s pattern — not the broader `canEditJob`.
...
2. Each is **reassigned in place, never deleted** (2026-09-14; `services/pipeline-ownership.js` `transferCustomerTo` → `reassignPipelineRow`): `UPDATE customer_pipeline SET sales_id=<new>` on the SAME row, plus a `pipeline_history` owner-change marker (`from_stage = to_stage`). The `customers` rows (and the `quotes` + `customer_interaction_updates` that CASCADE from them), `pipeline_history` and `pipeline_delete_requests` (CASCADE from the pipeline row) all keep their ids. `customers.user_id` is NOT rewritten — it records who made each contact (= `reports.user_id`, what `stats.js` counts KPIs by), just as past jobs keep their `sales_id`; the new owner reaches the thread via `customers.pipeline_id`, which every pipeline read and ownership check joins on. If the new sales already owns a row for the company, the other row is left untouched (never merged or deleted). (Until 2026-09-14 this step hard-deleted both rows, wiping the customer's quotes and interaction thread.)
...
**Confirm — server-driven** (2026-09-14): POST, and PUT `/:id` on a genuine sales change, answer `409 OWNER_CHANGE_CONFIRM_REQUIRED` `{customer_name, current_owners[], new_owner}` whenever the save would move the customer to another sales — however the name was entered (search pick OR typed). `CreateJobModal` / `JobDetailModal` show the shared `OwnerChangeConfirm` dialog naming both owners and re-send with `confirm_owner_change: true`. The "Data khách hàng" admin PATCH reassigns the same way (`409 OWNER_ALREADY_HAS_CUSTOMER` in the merge case). Past jobs keep their `sales_id`; PUT's L36 re-assert touches only the CURRENT owner's row — it never re-creates a row for the sales of an old job.
**⚠ A TRANSFER MOVES THE CUSTOMER ONLY — JOBS DO NOT FOLLOW. This is a deliberate product decision, not an oversight.** Transferring a customer to a new sales user rewrites `customer_pipeline.sales_id` (and with it the pipeline row, its history, and the whole `customers` → quotes / interaction thread hanging off it). It does **not** touch `jobs.sales_id` on any existing job — not pending ones, not completed ones.
- **Completed jobs ALWAYS stay with whoever did them.** `jobs.sales_id` is the historical record of who earned that shipment: it drives the Sales revenue-tick queue (SALES_REVENUE_TICK), `GET /api/jobs` role scoping (L22) and every KPI attribution. Rewriting it would retroactively move another rep's finished work — and their numbers — onto the new owner.
- **Pending jobs also stay with the old sales.** They are reassigned **one at a time** through the job's own "Sales" field in `JobDetailModal` (`PUT /api/jobs/:id`, `sales_id`). Each such edit fires the same server-driven `409 OWNER_CHANGE_CONFIRM_REQUIRED` confirm as a create does (whenever the customer is currently owned by someone else), so moving a job deliberately re-confirms the customer's ownership at the same time.
- **Why it is not cascaded:** "who owns the customer relationship going forward" and "who did this specific shipment" are two different facts, and only the first one is what a transfer decides. A cascade would silently rewrite history across an unbounded number of rows behind a single dropdown change, with no per-job review — the same class of damage the 2026-09-14 delete→reassign fix was made to stop. It is also exactly why `customers.user_id` is not rewritten on a transfer (it records who made each contact).
**Do not "fix" this later by making jobs follow the transfer.** If a rep's pending jobs genuinely need to move, that is N deliberate job edits, each individually confirmed — by design.
**Rules:** the sales user is the *owner*, not the company (never query "all pipelines for company X" without `sales_id`) · `(sales_id, LOWER(company_name))` UNIQUE is canonical; the transfer match is name-OR-FK · `xmax=0` on RETURNING detects insert-vs-update without a second query · **a sales change never deletes a `customers` or `customer_pipeline` row** (quotes, the interaction thread and history cascade from them) — reassign `customer_pipeline.sales_id`, and never rewrite `customers.user_id` on a transfer · **a transfer moves the CUSTOMER only — jobs never follow it** (see above).
...
A Hải Phòng **tk/both** job's OPS **thông quan** `job_ops_task` can be skipped. **The create-form checkbox + luồng paths (A/B) affect thong_quan ONLY; the reassign-dropdown "— Không cần —" (PATH C, 2026-07-21) also drops đổi lệnh. Non-HP jobs untouched.** Single source of truth for A/B = **`backend/src/services/ops-thongquan.js`** (`thongQuanWanted` predicate + idempotent `reconcileThongQuanTask`), imported by create + reconcile + the luồng-update endpoint (L30).
...
- **PATH C (reassign-dropdown "— Không cần —", 2026-07-21) — thong_quan + đổi lệnh:** the OPS thong_quan / đổi lệnh reassign dropdowns (`ReassignModal.jsx`, `canDrop = type==='ops' && taskType∈{thong_quan,doi_lenh}`) gained a "— Không cần (bỏ việc này) —" option (**OPS-only — CUS untouched**). Picking it (`ops_id`=null) → **`PATCH /api/jobs/:id/ops-task/:taskType/assign`** DROPS the task: `DELETE` the `job_ops_task` row so the completion gate no longer waits on it (same "no task" state as PATH A). **thong_quan** also sets `skip_ops_thongquan=TRUE` (persists past the luồng reconcile); **đổi lệnh** is delete-only (no flag — a later `service_type`/`destination` edit re-derives it via `reconcileJobSides` `dlDesired`). `ops_hp` cannot be dropped (it IS the job → 400). **Reversible:** assigning a real OPS person to a dropped (absent) task RE-CREATES it for them (INSERT) + un-skips thong_quan. **L35 keep-guard:** a task with OPS work (`completed`/`cost_entered_at`) is NOT hard-deleted — the drop is rejected 400 ("OPS đã có dữ liệu … không thể bỏ"), keeping the row (no cancel column, per no-schema-change). Display: `OpsTaskCell` shows a clickable **"Không cần"** for a dropped task (derived from job shape — thong_quan⇐HP tk/both, đổi lệnh⇐HP truck/both FCL); `deptStatusLines` truck-branch đổi lệnh lines now `dl &&`-gated (no phantom "Chưa đổi lệnh"). OPS overdue/stats/drilldowns are EXISTS/FILTER on the task row → a dropped task auto-excluded.
- **Completion gate UNCHANGED:** `checkOpsTasksDone` counts EXISTING rows (`tq_task_id !== null`, `dl_task_id !== null`) — a dropped/skipped task simply isn't there, so the job completes on the remaining work (or outright for tk-only skipped). The DD truck-complete gate `COALESCE(…doi_lenh…, TRUE)` treats an absent đổi lệnh as done. No gate edit needed.
...
### Note — Planning-mail subject line: labelled HBL/MBL segment + the DUPLICATED job SELECT (2026-09-10)
`renderSubject` (`backend/src/services/email-sender.js`) builds the carrier mail subject for `mail_type ∈ ('new','cancel')`. Current shape — the **BL segment sits between the customer and the cargo count**, so the job code + customer stay at the front of a mobile-truncated subject:
```
{ĐẶT|HỦY ĐẶT} KẾ HOẠCH XE {job_code} - {customer short name}[ - {HBL|MBL} {number}] - {cargo} / {Nhập|Xuất} / {DD/MM}
```
- **Customer segment = `customerShort(customer_name)`, not the first word (2026-09-14).** The old `firstWord()` rendered `ÂU LẠC` as `ÂU` and collapsed the 92 live customers into 77 labels (`CÔNG` alone stood for six companies); now 91. Order: NFC + trim + collapse spaces → leading legal-form words (`CÔNG TY`, `CTY`, `TNHH`, `CỔ PHẦN`, `MTV`, `DNTN`, …) **always** dropped → generic sector words (`XUẤT NHẬP KHẨU`, `THƯƠNG MẠI`, `SẢN XUẤT`, `DỊCH VỤ`, …) dropped **only while still over budget** (so `SX VINA` stays `SX VINA`) → whole words up to `CUSTOMER_NAME_BUDGET = 20`, first word kept whole even if longer, **no ellipsis**. Boilerplate matching is diacritic- and case-insensitive; the original text is what renders. Impact on the 289 sent mails: subject avg 67.1→71.0 chars, max 97→109; the customer segment now ends at char ~37 (p95 44) instead of ~33 (p95 36).
- **The segment is LABELLED `HBL `/`MBL `** so the carrier can tell which document the number refers to. Real BL numbers do not carry the prefix themselves (live: `SZHPH26006587`, `ONEYRTMG08553600`), so without the label the number is ambiguous on arrival.
- **HBL wins when both columns are set** (`blNumber(job)`, one home, ~L169). House bill first, master bill only as the fallback; the two are never both printed.
- **Blank-after-trim counts as ABSENT — and that matters because the column pair genuinely mixes `NULL` and `''`.** Live at ship time (417 non-deleted jobs): `hbl_no` = 213 NULL / **75 empty string** / 129 real; `mbl_no` = 273 NULL / **139 empty string** / 5 real. A `!= null` or plain truthiness test on the raw column would therefore emit ` -  - ` (an empty labelled segment) on ~200 jobs. `blNumber` does `String(... || '').trim()` and returns `''`, so **the segment AND its ` - ` separator both vanish** and a job with neither number renders **byte-identically to before the segment existed** — verified: 59 bytes, `before === after === live` for `LG26090055`.
- **⚠ THE JOB SELECT IS DUPLICATED — `sendPlanningEmail` (~:418) and `previewPlanningEmail` (~:761) each run their OWN copy** of the same `SELECT id, job_code, customer_name, han_lenh, import_export, cargo_type, so_kien, kg, cbm, hbl_no, mbl_no, NULL::text AS shipping_line FROM jobs WHERE id = $1 AND deleted_at IS NULL`. The comment on the preview copy says "identical query to send path" — that is an instruction, not a guarantee. **Any future field added to the subject (or body) MUST be added to BOTH copies.** Patch only the send copy and the mail goes out correct while "Xem mail" silently renders the field as `undefined` — no error, no crash, just a preview that disagrees with what the carrier receives, which is the worst possible failure mode for a preview button. (An L30 dedup into one shared `fetchJobForMail(jobId)` is the real fix — **deferred, not done**.)
- **What makes preview-vs-send divergence detectable:** there is exactly ONE `renderSubject` definition (~:177) and its two call sites (~:532 send, ~:822 preview) pass **byte-identical argument blocks**. So the SELECT is the *only* thing that can make the two paths differ — which is precisely why it is the thing that must stay in sync. Verification recipe used: run both SELECT texts against the live DB for the same job and deep-compare the rows, then call `previewPlanningEmail` for one job of each case (HBL-only / MBL-only / both / neither).
- **Schema:** `jobs.hbl_no` + `jobs.mbl_no`, both `VARCHAR(100)` nullable (`schema.sql` :419-420) — no new column was added for this.

**Rules:**
1. Adding a field to the mail subject or body → add it to **BOTH** job SELECTs (send + preview) in the same commit, then prove it by calling the preview path, not just by reading the diff.
2. Any new optional subject segment must collapse **with its separator** when blank, and must treat blank-after-trim as absent — these columns are not consistently NULL-vs-empty.
3. Don't print HBL and MBL together; the label plus one number is the agreed shape.
