...
| EMAIL_SYSTEM | `/api/email/*` + `/api/users/me/gmail-setup` | `schema.sql` (`users.gmail_address`, `users.gmail_app_password_encrypted`, `users.gmail_display_name`, `email_history` table), `utils/encryption.js` (AES-256-GCM, key from `GMAIL_ENCRYPTION_KEY` env var = 64 hex chars), `services/email-sender.js`, `routes/email.js`, `routes/users.js`, `ChangePassword.jsx` Gmail card, `TruckPlanningModal.jsx` Vùng 2 send wiring. **CP1**: DB columns + `email_history` audit table (soft-deletable, JSONB `last_sent_data` snapshot for "có thay đổi" diff). **CP2**: AES-256-GCM helper + `/api/users/me/gmail-setup` GET/PUT/DELETE + extended `/change-password` page. **CP3**: nodemailer Gmail SMTP send via `POST /api/email/send-planning` (DD+TPL), `GET /api/email/history` (DD+TPL+lead), rendered Vietnamese subject + body for `mail_type ∈ ('new','cancel')`. **Domain whitelist**: `@gmail.com`, `@googlemail.com`, `@slbglobal.com` (Google Workspace). `email_cc` parsed via L16. SMTP creds AES-256-GCM at rest; transcript/log never see plaintext. **Manual attachments (2026-06-30)**: DD can attach up to **10 EXTRA files (≤15MB total, ANY type)** to a carrier mail at send time via `InvoiceRecipientModal`'s 📎 picker — sent **ALONGSIDE** the auto BBBG PDFs and **NEVER persisted** (multer `memoryStorage`, held in RAM, discarded after the request; not on disk, DB, or `email_history`). `multer` is mounted **ONLY** on `POST /api/email/send-planning` (`.array('attachments', 10)`, `fileSize` 15MB per file + a handler-enforced cumulative ≤15MB check) via the `handleAttachmentUpload` wrapper that maps multer errors → Vietnamese 400s; other email routes stay JSON. The route JSON.parses the stringified `invoice_info`/`booking_ids` multipart fields + coerces the `is_replacement`/`attach_bbbg` boolean strings (multipart sends all fields as strings). `sendPlanningEmail` service gained an `extraAttachments = []` param pushed into nodemailer's already-wired `attachments[]` AFTER the BBBG loop; `bbbg_attached` flag + returned `attachmentCount` use a `bbbgAttachedCount` captured before the push so they stay BBBG-only. Frontend: `InvoiceRecipientModal` picker (gated on the send flow via `showAttachToggle`; ≤10/≤15MB client guard disables Send), `TruckPlanningModal.fireSend` builds FormData only when files present (else unchanged JSON — zero-regression common case), `api/index.js sendPlanningEmail` posts multipart when given a FormData (axios v1 sets the boundary). **Deferred**: per-card real status logic + HỦY workflow trigger UI + edit-after-send notifications (CP5). |
...
**P3 — TP per-task OPS UI (2026-06-23, revised scope — NO "+ việc khác"):**
- **3 OPS columns** on the TP dashboard (`LogDashboardTP.jsx`), HP jobs only, driven by `j.ops_tasks[]` (P2 projection carries `ops_id`+`ops_name`): **Thông quan** (thong_quan) / **Đổi lệnh** (doi_lenh) / **Việc khác** (ops_hp). Each cell shows assignee + short status (chưa làm / chưa cost / xong); empty slot when the job has no task of that type. Reusable `OpsTaskCell` renders both desktop `cell()` + mobile card (L26). `ALL_COLS` key `ops` → `ops_tq`/`ops_dl`/`ops_vk` (loadVisibleCols auto-appends the new keys). JobDetailModal "Công việc OPS" gained a per-task-type label (owner already shown via P2).
- **Per-task reassign** (manual override of the rotation): click a filled cell → `ReassignModal` (extended with `taskType`/`taskLabel`) → **`PATCH /api/jobs/:id/ops-task/:taskType/assign`** (TP-only, `validateAssignee ['ops']` = L32). Sets ONLY that task's `job_ops_task.ops_id` (+`assigned_at`/`by`); resyncs the cosmetic `ja.ops_id` primary pointer via `resyncJaOpsPrimary` (doi_lenh→thong_quan→ops_hp COALESCE); notifies the new owner. Does NOT touch the job's other tasks.
- **"+ đổi lệnh"** on an empty doi_lenh cell of a tk-only HP job → **`POST /api/jobs/:id/ops-task` {task_type:'doi_lenh'}** (repurposed; old free-text/TP-picked behavior removed, had zero callers). Auto-assigns THIS WEEK's `doiLenhOpsId` via `getWeekRotation` (TP does NOT pick — L30 single source); `ON CONFLICT (job_id,task_type) DO NOTHING` → 400 if it already exists. New doi_lenh now gates completion. NO "+ việc khác" button — "việc khác" is the `ops_hp` service_type chosen at job creation, never added later.
- **reassign-ops RETIRED:** `PATCH /:id/reassign-ops` deleted (its whole-job wipe-&-recreate destroyed per-task owners). Frontend `ReassignModal` OPS path + `reassignOps` API client removed; only `ReassignModal` CUS (job-level) remains. TP OPS reassignment goes through the per-task endpoint.
- **L30 cleanup:** `assignOps` removed (dead). `suggestOps`+`getOpsContext` KEPT — still the null-rotation fallback in create-seeding (`jobs.js ~1451`) + the `waiting_ops` suggestions endpoint. `assignCus` left in place (also dead; retire with future CUS-rotation work). `ja.ops_id` NOT dropped (still the cosmetic primary pointer + `/overview` staff-distribution).
- **Deferred (not in P3):** dropping `ja.ops_id` entirely + flipping the `/overview` staff-distribution ops branch to per-task; the 3 historical tk-only jobs (#55/#61/#64) still carry a `doi_lenh` task (untouched, pending a one-off cleanup decision).

**LCL rule (2026-06-24) — LCL shipments don't auto-get a `doi_lenh`:** an **LCL** (`jobs.cargo_type='lcl'`) shipment to Hải Phòng does NOT auto-create a `doi_lenh` task, even when `service_type` is truck/both — same shape as the tk-only rule but gated on `cargo_type` instead of `service_type`. **FCL** keeps today's behavior (truck/both at HP auto-gets `doi_lenh`). TP adds `doi_lenh` to an LCL job later via the **"+ đổi lệnh"** escape hatch (rotation-assigned, like tk-only). 4 sites (L31 lockstep): ① create-seeding `jobs.js:~1710` (`&& cargo_type !== 'lcl'`); ② `reconcileJobSides` `dlDesired = newHP && truckDesired && !isLcl` (reads `cargo_type` fresh from the job inside the helper — not threaded through `PUT /:id`); ③ "+ đổi lệnh" POST left as-is (the manual escape hatch — no LCL exclusion); ④ `OpsTaskCell` "+ đổi lệnh" button now shows for `service_type==='tk' OR cargo_type==='lcl'` (shared desktop+mobile, L26). Plus: `primaryOpsOwner`/`resyncJaOpsPrimary` set the cosmetic `ja.ops_id` to the `thong_quan` owner on an LCL both (no `doi_lenh` person to point at), and only the `thong_quan` owner is notified. Completion gate unchanged (EXISTS-tolerant → LCL both completes on `thong_quan` + tk/truck). 5 in-flight LCL both jobs (#46/#54/#57/#73/#80) hand-fixed by the owner.
