...
1. **Reconcile regression proof never completed.** Three attempts at invoking the actual `PUT /:id` handler in-process (extracting the registered function directly, intercepting `COMMIT` to prevent persistence) stalled indefinitely at the same point — after acquiring a DB client, before the first `client.query()` — for an unconfirmed reason; a minimal harness sanity-check proved the interception mechanism itself works, so the stall is specific to the handler invocation. Real-world DD usage over the coming weeks is the substitute proof for now.
2. **`cont_number`-edit failure mode, undiagnosed.** Hypothesis: editing only the `cont_number` of a container that already has a live booking may not match on content (`cont_number`/`cont_type`/`seal_number` signature), causing the reconcile to treat the old row as unmatched (preserved, since it's linked) AND insert the new number as a fresh row — leaving the job with 2 container rows and the booking still pointing at the OLD number (stale on BBBG). Not tested; do not assume fixed.
3. **Only `PUT /api/jobs/:id` was audited/fixed for the cascade-delete pattern.** Other code paths that might delete `job_containers` rows (if any exist elsewhere) have not been swept for the same `ON DELETE CASCADE` exposure.
4. **A second, structurally identical instance of the Bug-B proxy class, found but not fixed:** the DD "Kế hoạch đã đặt / chưa đặt" cards (`keHoachDaDat`/`keHoachChuaDat`, `routes/jobs.js`) use a container-level `EXISTS` on `truck_booking_containers` with **no `cargo_type` branch** — so a pending LCL job carrying a reference container row can still land in "chưa đặt" on this card even when `get_truck_booking_status()` correctly reports it as planned. Confirmed via live query; not fixed.
