...
| EMAIL_SYSTEM | _(CP1 — DB only, no API/UI yet)_ | `schema.sql` (`users.gmail_address`, `users.gmail_app_password_encrypted`, `users.gmail_display_name`, `email_history` table) — Phase 5 Step 3 Part 2 infrastructure for DD/TPL emailing kế hoạch xe to transport companies. **CP1 (this commit)** adds the schema only: per-user Gmail SMTP credentials (encrypted at rest) on `users`, and a soft-deletable `email_history` audit table that stores per-send-attempt snapshots (sender + recipient + full CC list + rendered subject/body + the bookings JSONB used so "có thay đổi sau gửi" detection can diff against current `truck_bookings` state). `mail_type ∈ ('new','cancel')`, `status ∈ ('sent','failed')`. `booking_ids INT[]` groups N bookings into one mail. 4 partial indexes filter soft-deleted rows. **CP2+**: encryption helper, SMTP send via nodemailer, BBBG attachment generation, status logic ("Đã gửi" / "Có thay đổi sau gửi" / "Cần gửi HỦY"), HỦY workflow, and Gmail setup in `/change-password`. |
