// AirQuoteDisplay — compact read-only render of an air freight v2 quote
// (2026-05-27). Mirrors SeaQuoteDisplay; routes/dimensions are air-specific.
...
export default function AirQuoteDisplay({ quote }) {
  ...
  const activeBreaks = (qd.rate_breaks || []).filter(b => parseNum(b.kg) > 0).map(b => b.break);
  ...
  return (
    <div style={{ fontSize: 12, color: 'var(--text-2)', marginTop: 4 }}>
      ...
      <ChargeBlock title="Phí quốc tế" rows={tickedIntl} ctx={ctx} totals={intlT} activeBreaks={activeBreaks} />
      <ChargeBlock title="Phí nội địa" rows={tickedInland} ctx={ctx} totals={inlandT} activeBreaks={activeBreaks} />
...
function ChargeBlock({ title, rows, ctx, totals, activeBreaks }) {
  if (!rows.length) return null;
  const qtyByBreak = Object.fromEntries((ctx.rate_breaks || []).map(b => [b.break, parseNum(b.kg)]));
        ...
        <thead>
          <tr>
            ...
            {showBreakCols && activeBreaks.map(b => (
              <th key={b} colSpan={2} style={{ ...TH_S, textAlign: 'center' }}>{b}</th>
            ))}
          ...
          {rows.map((r, i) => {
            ...
            return (
              <tr key={i}>
                ...
                <td style={{ ...TD_S, textAlign: 'right',
                  color: !isKg && flatRate > 0 ? 'var(--text-2)' : 'var(--text-3)',
                  fontSize: 10.5, whiteSpace: 'nowrap' }}>
                  {isKg ? '—' : (flatRate > 0 ? fmtAmount(flatRate, cur) : '—')}
                </td>
