...
| `booked` | Won — ANY booked quote OR ANY job. **Permanent: never demoted by a newer interaction.** |
...
1. `new`/`following` → `dormant`: `last_activity_date < CURRENT_DATE - INTERVAL '7 days'` **AND the pipeline row has ZERO linked jobs** (2026-07-31 fix — see below).
...
**"Has ever had a shipment ⇒ never dormant" (2026-07-31).** A customer with ANY job (completed, pending, or even cancelled/soft-deleted — the mere existence of a job row counts) is permanently exempt from the dormant demotion, via a `NOT EXISTS (SELECT 1 FROM jobs j WHERE j.customer_id = cp.customer_id OR LOWER(j.customer_name) = LOWER(cp.company_name))` guard added to the transition-1 candidate query. The 7-day-stale rule now applies **only to genuine zero-job leads**. Root cause this fixes: `last_activity_date` only advances on Sales-side report saves (never on LOG-side job creation/edits), so a customer with live, ongoing shipments but no *fresh Sales report* would silently age past 7 days and get dormant-flipped despite active work — discovered via the "THT Tiêu Dùng" customer-not-findable bug report (5 jobs, one still pending, yet `stage='dormant'`).

**Companion fix — `PUT /api/jobs/:id` now re-asserts `stage='booked'`.** Before this fix, only `POST /api/jobs` (job creation, the "L14" upsert at `jobs.js:~1638-1673`) touched `customer_pipeline`; editing an existing job never did. Now PUT mirrors the same upsert (`jobs.js`, just before `COMMIT`) — any successful edit re-asserts `stage='booked'` + refreshes `last_activity_date = CURRENT_DATE` for the job's customer, using the row's FINAL `sales_id`/`customer_name`/`customer_id` post-update (not `req.body`, since an edit may touch unrelated fields). This is a safety net alongside the dormant-guard above — investigation showed neither guard alone fully closes the gap (a third contributing cause, `db/backfill_pipeline.js`'s deploy-time stage recompute — which only recognizes Sales-side `quotes.status='booked'`, not jobs — was identified but is NOT fixed by this change; it can still silently re-demote a job-only-booked customer on a future deploy since its `booking` CTE doesn't check `jobs`). **Fixed 2026-09-14:** backfill Step 0 now never demotes a job-bearing customer out of `booked`, restores rows an earlier deploy had demoted, and logs every stage change to `pipeline_history` — same `hasAnyJobSql` predicate as the dormant guard (`services/pipeline-ownership.js`).

**Stage is derived PER QUOTE (2026-09-15).** The stage table above reads top-down as a priority list, not as "whatever the newest interaction says": **`booked` = ANY booked quote in the customer's thread OR ANY job, and it is permanent** — a later quote, or a later contact the rep marks `contacted`/`quoted`, describes only THAT quote/contact and must never pull the customer's own stage back down. `new`/`following`/`dormant` are decided by the most recent `interaction_type` **only for customers that have never had a booked quote and never had a job**. In `backfill_pipeline.js` Step 0 both halves are now explicit predicates from `services/pipeline-ownership.js` — `hasAnyJobSql` **OR** `hasBookedQuoteSql` — instead of the booked half being left to `correct_stages`' `booking` CTE, which is unreachable for a row whose reports are all soft-deleted. Step 1 (INSERT) needs no equivalent: `correct_stages` already tests `b.is_booked` first, and only Step 0 can demote. Verified on live data: 0 of 233 rows differ between the job-only rule and the widened rule, and the 132 rows with neither fact are unchanged.

**One-time backfill (2026-07-31, run once, not idempotent-by-design):** `UPDATE customer_pipeline SET stage='booked' WHERE stage='dormant' AND deleted_at IS NULL AND EXISTS(...same job-match...)` corrected 2 pipeline rows wrongly stuck `dormant` despite active jobs — **SKYBOW** (id 6997, 7 jobs) and **THT TIÊU DÙNG** (id 7259, 5 jobs, one still pending). `last_activity_date` was left untouched (irrelevant now that the guard exempts them going forward).
